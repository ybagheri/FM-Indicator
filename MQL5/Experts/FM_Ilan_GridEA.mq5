...
input group "=== FM Measured-Move Sizing (only if InpEntryMode = ENTRY_FM_*; overrides the first trade's TP/SL) ==="
input bool              InpUseMMSizing            = false;  // false = unchanged behavior (InpInitialTPPoints, no SL until grid-end)
input ENUM_MM_RR_MODE   InpMMRRMode               = MM_RR_DOUBLE; // 1:1 (stop=66% MM) or 1:2 (stop=33% MM)
input double            InpMMMinStopPoints        = 500.0;  // floor on the MM-derived stop distance
...
double            g_fm_mm_range = 0.0; // v1.3: matching setup's raw MM size (price units), 0=none found
...
void ManageBreakEven()
{
   ...
   // v1.3: while there is exactly ONE position and it was sized by
   // InpUseMMSizing (OpenInitial set its own tight MM-based SL/TP), don't
   // let the wide grid-boundary stop / InpInitialTPPoints TP overwrite it.
   // Once a second grid level fills (count>1 — martingale averaging has
   // actually started), the normal wide basket stop takes over as usual.
   bool mmOwnsFirstTrade = (InpUseMMSizing && IsFMEntryMode(InpEntryMode) &&
                            count == 1 && g_fm_mm_range > 0.0);
...
bool OpenInitial(const ENUM_POSITION_TYPE direction)
{
   ...
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double entry_ref = (direction == POSITION_TYPE_BUY) ? ask : bid;

   double sl = 0.0;
   double tp = 0.0;

   // v1.3: if the signal came from the FM engine and carried a matched raw
   // MM size, size THIS trade's stop/target off that (% of MM, floored),
   // instead of the plain InpInitialTPPoints/no-SL default below. Grid
   // levels + lot schedule (LotForLevel) are unaffected either way — this
   // only ever touches the very first trade's own SL/TP.
   if(InpUseMMSizing && IsFMEntryMode(InpEntryMode) && g_fm_mm_range > 0.0)
   {
      int dir = (direction == POSITION_TYPE_BUY) ? 1 : -1;
      double mmRangePoints = g_fm_mm_range / PointValue();
      ...
      sl = NormalizePrice(entry_ref - dir * slPoints * PointValue());
      tp = NormalizePrice(entry_ref + dir * tpPoints * PointValue());
   }
   else if(InpInitialTPPoints > 0.0)
   {
      if(direction == POSITION_TYPE_BUY)
   ...
   bool ok = false;
   if(direction == POSITION_TYPE_BUY)
      ok = trade.Buy(lot, _Symbol, 0.0, sl, tp, "FM_Ilan Initial BUY");
   else
...
int FMStrategySignal()
{
   ...
   if(dir != 0 && InpUseMMSizing)
   {
      // Re-derive the raw MM size from the matching active snapshot's a0/a1
      // prices — no new struct fields needed: dir + objective (b0_price) are
      // copied verbatim through SetupPlan/GeneralSetup, so this match is
      // exact. See docs/RISK_MANAGEMENT.md §5 / docs/SESSION_MM.md §4.
      FMSetupSnapshot snaps[];
      int n = g_fm_analysis.EnginePtr().ActiveSnapshots(snaps);
      for(int i = 0; i < n; i++)
      {
         if(snaps[i].dir != sel.setup.dir) continue;
         if(MathAbs(snaps[i].b0_price - sel.setup.objective) > PointValue() * 2) continue;
         g_fm_mm_range = MathAbs(snaps[i].a1_price - snaps[i].a0_price);
         break;
      }
   }

   if(InpFMVerboseLog && dir != 0)
      PrintFormat("[FM_Ilan_GridEA] %s %s dir=%s entry=%s score=%d R=%.2f%s mm_range=%s",
                  ...
                  DoubleToString(g_fm_mm_range, _Digits));
