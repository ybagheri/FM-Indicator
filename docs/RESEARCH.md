...
| 4 | Which MM variant? (Leg1=Leg2, range height, channel, gap, spike+channel, inverse…) | v1 implements **Leg1=Leg2 (AB=CD)** + optional **inverse-MM from failed BO** flag. v1.2 adds **range-height**, **shallow-pullback channel**, and **measuring-gap** families (flag-gated, default OFF; `CRangeHeightMM`/`CChannelMM`/`CGapMM`, SPEC §4b) — this row's "NOT implemented" is superseded; kept as the decision record. |
